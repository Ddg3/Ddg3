...
public class GameManager extends AbstractGame
{
    ...
    public static int pauseInd = 1;
    private int controlsInd = 1;
    private boolean stickSelecting = false;
    private boolean creditRolling = false;
    private Object creditFadeAway;
    private ArrayList<TextObject> credits;
    ...
    public void init(Main main)
    {
        ...
        pauseUI.add(4, new Object("Credits", 162, 40, "/blockNumbers.png", 2, 0.01f));
    ...
    public void update(Main main, float dt)
    {
        ...
        if(!isPlaying)
        {
            if(!creditRolling) {
            ...
            else
                {
                    ...
                    for(int i = 0; i < templatePedestals.size(); i++){
                        templatePedestals.get(i).visible = false;
                    }
                    for(int i = 0; i < timePedestals.size(); i++){
                        if(timePedestals.get(i) != null)
                            timePedestals.get(i).visible = false;
                    }
                    for(int i = 0; i < templateText.size(); i++) {
                        templateText.get(i).visible = false;
                    }
                }
        }
        else
        ...
        {
            if(i == 0)
                pauseUI.get(i).setPosition(center.getPosition().x, center.getPositionY() - 120);
            else if(i == 5)
                pauseUI.get(i).setPosition(center.getPosition().x, center.getPositionY());
            else
                pauseUI.get(i).setPosition(center.getPosition().x, center.getPositionY() -85 + (i * 55));
        }
        if(creditRolling)
        {
            if (creditFadeAway.getFrame() == 0)
            {
                ...
                if(credits.size() > 0)
                {
                    ...
                    {
                        credits.get(i).posY -= 1;
                        if(credits.get(i).posY <= center.position.y){
                            credits.get(i).visible = false;
                            credits.remove(i);
                        }
                    }
                }
                    ...
                    {
                        if(creditFadeAway.getFrame() == 0) {
                            creditFadeAway.playTo(0, 5);
                        }
                }
            }
            else if(creditFadeAway.getFrame() >= 4 && credits.size() == 0) {
                    for(int i = 0; i < pauseUI.size(); i++) {
                        pauseUI.get(i).visible = true;
                    }
                    for(int i = 0; i < templatePedestals.size(); i++){
                        templatePedestals.get(i).visible = true;
                    }
                    for(int i = 0; i < timePedestals.size(); i++){
                        if(timePedestals.get(i) != null)
                            timePedestals.get(i).visible = true;
                    }
                    for(int i = 0; i < templateText.size(); i++) {
                        templateText.get(i).visible = true;
                    }
                    for(int i = 0; i < pauseUI.size(); i++) {
                        pauseUI.get(i).visible = true;
                    }
                    objects.remove(creditFadeAway);
                    creditFadeAway = null;
                    creditRolling = false;
                    isPlaying = false;
            }
    ...
    {
        center.setPosition(0, GameLevelManager.currLevel.verticleBounds.get(camera.boundsRange).x);
        center.offsetPos.x = center.position.x;
        center.offsetPos.y = center.position.y;
        camera.setPosY(center.offsetPos.y);
        camera.setPosX(center.offsetPos.x);
        creditRolling = true;
        creditFadeAway = new Object("fadeAway", 640, 360, "/fadeAwayReverse.png", 6, 0.3f);
        objects.add(creditFadeAway);
        creditFadeAway.isActiveOnPause = true;
        creditFadeAway.zIndex = Integer.MAX_VALUE;
        creditFadeAway.setPosition(GameManager.center.position.x, GameManager.center.position.y);
        creditFadeAway.zIndex = Integer.MAX_VALUE - 1;
        creditFadeAway.setFrame(5);
        creditFadeAway.playToInReverse(5, 0);

        credits = new ArrayList<>();
        String[] text = {"Credits:", "", "Development:", "", "Lead Programming, Art, and Design: Zach Lawrence", "Additional Art and Design: Felix Zhao",
                "", "Main Testing and Feedback:", "", "Nirmal Kumar", "Roger Wang", "", "Special Thanks:", "", "Vikranth Mandadi",
                "Brandon Choi", "Alan Sun", "", "And thank you to my parents, friends, and family who supported me throughout this journey",
                "", "A final thanks to Majoolwip, his YouTube tutorials got me started on the game and gave me direction.",
                "Without him this game wouldn't exist"};

        for (int i = 0; i < text.length; i++) {
            credits.add(i, new TextObject(text[i], ((int)center.position.x + 10), ((int)center.position.y + center.height) + (i * 20), 0xffffffff, 1));
            textObjects.add(credits.get(i));
        }
    }
    public void pauseSelect(int pauseInd, Main main)
    {
        switch(pauseInd)
        {
            case 1:
