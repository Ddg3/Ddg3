...
public class CPU_Script2 extends Player
{
    ...
    private int newKeyDropIn = KeyEvent.VK_2;
    ...
    private int randomWeapon = ThreadLocalRandom.current().nextInt(0, 2);
    private int color = (int)(Math.random() * Integer.MAX_VALUE);
    private Object stars;

    public CPU_Script2(String name, int width, int height, String path, int totalFrames, float frameLife, int playerNumber)  /// Changed to fit constructor name
    {
        super(name, width, height, path, totalFrames, frameLife, playerNumber);  /// Added playerNumber argument
        ...
        weaponInit();
        weapon = (WeaponComponent) this.findComponent("Weapon");
    }
    ...
    public void cameraCollision()
    {
        if(this.position.x - (this.width / 2) < GameManager.center.position.x - (GameManager.center.width / 2))
        {
            ...
            {
                if(!GameManager.players.get(i).getCollidingRight())
                {
                    collidingRight = true;
                    return;
                }
        ...
        if(this.position.x + (this.width / 2) > GameManager.center.position.x + (GameManager.center.width / 2))
        {
            for(int i = 0; i < GameManager.players.size(); i++)
            {
                if(!GameManager.players.get(i).getCollidingLeft())
                {
                    collidingLeft = true;
                    return;
                }
        ...
        if(this.position.y + (this.height / 2) > GameManager.center.position.y + (GameManager.center.height / 2))
        {
            for(int i = 0; i < GameManager.players.size(); i++)
            {
                if(!GameManager.players.get(i).getCollidingTop())   /// Change to getCollidingTop() and add in Player.java
                {
                    collidingTop = true;
                    return;
                }
    ...
    public void moveKeyboard(Main main, float dt)
    {
        ...
        Vector playerVector = new Vector(player.getPositionX(), player.getPositionY());
        Vector targetVector = new Vector(target.getPositionX(), target.getPositionY());
        double angle = findAngle(playerVector, targetVector);
        // int minAngle = ((int)(angle / 45)) * 45;
        int minAngle = (int)(Math.floor(angle / 45) * 45);
        int maxAngle = (int)(Math.ceil(angle / 45) * 45);

        double diagonalSpeedIncrease;

        if (perpendicularDistance(player, target, minAngle) < perpendicularDistance(player, target, maxAngle)){
            diagonalSpeedIncrease = (minAngle % 90 == 0) ? 1 : Math.sqrt(2);

            this.position.x += 200f * Math.cos(Math.toRadians(minAngle)) * diagonalSpeedIncrease * dt;
            this.position.y += 200f * Math.sin(Math.toRadians(maxAngle)) * diagonalSpeedIncrease * dt;
        } else {
            diagonalSpeedIncrease = (maxAngle % 90 == 0) ? 1 : Math.sqrt(2);

            this.position.x += 200f * Math.cos(Math.toRadians(maxAngle)) * diagonalSpeedIncrease * dt;
            this.position.y += 200f * Math.sin(Math.toRadians(maxAngle)) * diagonalSpeedIncrease * dt;
        }
    ...
    public void lookKeyboard(Main main, GameManager gameManager)
    {
        ...
        /// 60 degree span on major axises and 30 degree on minor ok sure
        Vector playerVector = new Vector(player.getPositionX(), player.getPositionY());
        Vector targetVector = new Vector(target.getPositionX(), target.getPositionY());
        double angle = findAngle(playerVector, targetVector);
    ...
    {
        if((isKeyBoard && main.getInput().isKey(newKeyDropIn)) && !this.isInGame() && !this.droppingIn)
        {
