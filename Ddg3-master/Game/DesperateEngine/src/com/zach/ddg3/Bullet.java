...
public class Bullet extends Object
{
    ...
    public Bullet(String name, int width, int height, String path, int totalFrames, float frameLife, int direction, WeaponComponent weapon)
    {
        ...
        if(this.getObjImage().getPath() == "/weaponSelectBox1.png")
        {
            fullyCharged = true;
        }
        if(decelRate == 0)
        ...
        if(weapon.isAnimated() && chargeIndex == 0)
        {
            if(!fullyCharged)
            {
                ...
                {
                    playInRange(direction * 2, (direction * 2) + 2);
                }
        }
    ...
    {
        if(chargeIndex == 1 && !started)
        {
            stop();
            playInRange(direction * 6, (direction * 6) + 6);
            started = true;
        }
        else if(chargeIndex == 2 && !started)
        {
            stop();
            int newDir = 0;
            switch(direction)
        ...
        if(!isAlt)
        {
            ...
            if(weapon.isPlanting() && this.speed != 0)
            {
                plant();
                ((Player)owner).rumble(40000, 0.1f);
